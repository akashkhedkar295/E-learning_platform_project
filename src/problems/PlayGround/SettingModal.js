...
const SettingsModal=(props) => {
    ...
    const handleClickDropdown = (e) => {
		e.stopPropagation();
		props.setSettings({ ...props.settings, dropdownIsOpen: !props.settings.dropdownIsOpen });
	};
	return (
			...
			<div aria-modal='true' role='dialog' className='fixed inset-0 overflow-y-auto z-modal'>
				<div className='flex min-h-screen items-center justify-center px-4'>
					...
					<div className='opacity-100' onClick={() => {
                         props.setSettings({ ...props.settings, settingsModalIsOpen: false})
                    }}>
											...
											>
												{EDITOR_FONT_SIZES.map((fontSize, idx) => (
													<SettingsListItem
														key={idx}
														fontSize={fontSize}
														selectedOption={props.settings.fontSize}
                                                        handleFontSizeChange={(fontSize) => {
															setFontSize(fontSize);
															props.setSettings({ ...props.settings, fontSize: fontSize });
														}}
													/>
												))}
											</ul>
